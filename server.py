...
# ── PIN auth (hardcoded SHA-256 of "3102") ───────────────────────────────────
import hashlib
PIN_HASH = hashlib.sha256(b"3102").hexdigest()  # pre-computed
...
# ── Auth middleware ──────────────────────────────────────────────────────────
PUBLIC_PATHS = {"/health", "/", "/opnote", "/cohort", "/auth/pin", "/auth/status", "/prep"}
...
class PinAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Check Authorization header, then cookie fallback (sendBeacon)
        token = None
